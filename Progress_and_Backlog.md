...
**Экономика действий** закрыта (Фаза 11.5). Текущий приоритет: **Dice Engine**, рефакторинг **ConfigModal** (см. беклог). Плагин `logic.py` на уровне системы (ADR-12) остаётся в дорожной карте для кастомных правил.
