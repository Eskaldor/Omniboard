...
- [x] **Массовый бросок:** несколько упоминаний `@ИмяАктёра` → последовательные `POST /api/combat/actors/{id}/roll` через `Promise.all` с одним и тем же выражением и комментарием.
- [x] **Универсальный бросок:** `POST /api/combat/roll` без акторского контекста (системный бросок Мастера).
- [x] **Справка `?`:** краткая подсказка по синтаксису во время ввода в режиме Roll.
