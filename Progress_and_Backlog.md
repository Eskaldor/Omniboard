...
**Экономика действий** (Фаза 11.5) и **архитектурная декомпозиция стейта / данные вне боя** (Фаза 11.8) закрыты. **Текущая фаза: Phase 12 — Dice Engine** (модульные броски, интеграция с треком и логом). Параллельно в беклоге: рефакторинг **ConfigModal**. Плагин `logic.py` на уровне системы (ADR-12) остаётся в дорожной карте для кастомных правил.
...
- [ ] **ConfigModal:** разбиение на **табы** (система / колонки / таблица / язык и т.д.) и декомпозиция компонента — модалка разрослась и усложняет сопровождение.
