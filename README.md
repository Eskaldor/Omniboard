...
- **Бросок инициативы по системе** — шаблон `initiative_roll` в `mechanics.json` (`[stat_key]`, fallback `1d20`, или `none` без кубика); массовый бросок и настройки в **GM Console** (см. `Omniboard_TZ.md` §2.1.5)
...
- **Шаблоны листа** (`sheet_profiles`): группировка характеристик и макросов (аккордеоны `stats/actions`), плюс выбор **кастомного React-листа** через `custom_component_id` (Foundry-style `SheetRegistry`) или универсальный вид (см. `Omniboard_TZ.md` §2.1.3)
- **Броски и обратная связь**: результат (`total`, разбор кубиков в `details`) показывается во всплывающих уведомлениях в стиле интерфейса; журнал боя остаётся полным архивом (см. `Omniboard_TZ.md` §2.7). Опциональная колонка макросов в таблице инициатив — §2.1.4.
- **Скрытые броски и «шёпот» игрока мастеру**: `is_secret` в логах, фильтрация в Player View, отдельные тосты у мастера
...
6. **Консоль мастера (нижняя панель):** в режиме **Roll** можно вводить броски с токенами `@` / `!` / `$` и сегментами `;` — см. кратко [readme_beta.md §10](readme_beta.md) и полный контракт [Omniboard_TZ.md §2.6](Omniboard_TZ.md).
