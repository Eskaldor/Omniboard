...
def entry_dict_to_md_line(entry: dict) -> str:
    """Single combat log entry → Markdown line (shared by live writer and archives)."""
    t = entry.get("type", "")
    r = entry.get("round", 1)
...
def combat_history_archive_markdown(history: Sequence[LogEntry]) -> str:
    """Full session log as Markdown for disk archive (ADR-18 / end-of-combat rotation)."""
    snapshots = [h.model_dump(mode="json") for h in history]
    md_lines = [entry_dict_to_md_line(e) for e in snapshots]
    body = "\n".join(md_lines).strip()
    return (body + "\n") if body else "\n"


def combat_history_archive_json(history: Sequence[LogEntry]) -> str:
    """Full session log as JSON text for disk archive."""
    ...
    *,
    ...
    def _write() -> None:
        try:
            ...
            md_lines = [entry_dict_to_md_line(e) for e in history_snapshot]
