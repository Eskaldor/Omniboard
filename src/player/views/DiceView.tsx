...
import { Loader2, Dices, Delete, Eye, EyeOff } from 'lucide-react';
...
import { normalizeRollResult, parseRollHttpResponse, showRollErrorToast, showRollResultToast } from '../../utils/rollToast';
  ...
  return (
    <div className="absolute inset-0 z-10 pointer-events-none">
      ...
      <svg width={sz} height={sz} className="absolute inset-0">
        ...
        {/* 10 tick marks evenly around the ring */}
        {Array.from({ length: 10 }, (_, i) => {
          const a = (i / 10) * 2 * Math.PI - Math.PI / 2;
          const inner = arcR - strokeW / 2 + 5;
          const outer = arcR + strokeW / 2 - 5;
          return <line key={i}
            x1={half + inner * Math.cos(a)} y1={half + inner * Math.sin(a)}
            x2={half + outer * Math.cos(a)} y2={half + outer * Math.sin(a)}
            stroke="#3f3f46" strokeWidth="2.5" />;
        })}
      </svg>

      {/* Handle: die icon riding the ring */}
      <div
        className="absolute rounded-full border-2 border-amber-400/80 bg-zinc-900 flex items-center justify-center text-amber-300 shadow-xl"
        style={{ width: handleSz, height: handleSz, left: hx - handleSz / 2, top: hy - handleSz / 2 }}
      >
        <DiceIcon die={die} size={iconSz} />
      </div>

      {/* Center: count display (replaces toggle during radial) */}
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 select-none">
        <span className="text-5xl font-bold text-zinc-100 tabular-nums leading-none">{count}</span>
        <span className="text-sm font-semibold text-amber-400 leading-none">{die}</span>
        {tens > 0 && (
          <span className="text-xs text-zinc-500 mt-1">
            {tens}&thinsp;{tens === 1 ? 'оборот' : tens < 5 ? 'оборота' : 'оборотов'}
          </span>
        )}
      </div>
    </div>
...
export function DiceView({ auth, state, rollRequestStatus }: Props) {
  const { t } = useTranslation('core', { useSuspense: false });
  const { actor, loading } = usePlayerActor(auth, state);
  const [expression, setExpression] = useState('');
  const [rolling, setRolling] = useState(false);
  const [isSecret, setIsSecret] = useState(false);
  const [pendingId, setPendingId] = useState<string | null>(null);
  const [mode, setMode] = useState<'dice' | 'numpad'>('dice');

  // Radial ring state — replaces all die buttons while active
  const [radialState, setRadialState] = useState<{ die: DieType; count: number; startAngle: number } | null>(null);
  ...
  useLayoutEffect(() => {
    ...
    if (!el) return;
    setSz(Math.floor(el.offsetWidth));
    const ro = new ResizeObserver(([e]) => setSz(Math.floor(e.contentRect.width)));
    ro.observe(el);
    return () => ro.disconnect();
  }, [loading]); // re-run when loading resolves so circleRef is in the DOM

  // Long-press refs
  const longPressTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const longPressActive = useRef(false);
  const pressCancel = useRef(false);
  const startPos = useRef({ x: 0, y: 0 });

  // Radial tracking refs
  const radialPrevAngle = useRef<number | null>(null);
  const radialAccum = useRef(0);
  const radialBase = useRef(0);
  const radialSnapCount = useRef(0);
  const pressInfo = useRef<{ el: HTMLButtonElement; pid: number; die: DieType } | null>(null);

  // Live half for use in event-handler closures (avoids stale sz capture)
  const halfRef = useRef(sz / 2);
  halfRef.current = sz / 2;

  // Live expression ref so long-press timer can read current value
  const expressionRef = useRef(expression);
  expressionRef.current = expression;

  // Numpad pause state
  const numpadTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const numpadActiveRef = useRef(false);

  const canRoll = useMemo(() => expression.trim().length > 0 && !rolling, [expression, rolling]);
    ...
    try {
      if (!canRollNow) {
        const res = await fetch('/api/player/roll/request', {
          ...
          body: JSON.stringify({ expression: expr, comment: expr, is_secret: isSecret }),
        });
        const raw = (await res.json().catch(() => ({}))) as Record<string, unknown>;
        const status = typeof raw.status === 'string' ? raw.status : '';
        const request_id = typeof raw.request_id === 'string' ? raw.request_id : '';
        if (status === 'pending' && request_id) { setPendingId(request_id); return; }
        if (status === 'approved') {
          const result = normalizeRollResult(raw.result);
          if (!result) { showRollErrorToast('Не удалось выполнить бросок'); return; }
          showRollResultToast({ result, actorName: actor?.name, comment: isSecret ? t('player.roll.secret', 'Скрытый бросок') : expr });
          hapticTap('light');
          return;
        }
      ...
      const res = await fetch(`/api/combat/actors/${encodeURIComponent(auth.actorId)}/roll`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Player-Token': auth.token },
        body: JSON.stringify({ expression: expr, is_preroll: false, is_secret: isSecret }),
      });
      const parsed = await parseRollHttpResponse(res);
      if (!parsed.ok) { showRollErrorToast((parsed as { ok: false; message: string }).message); return; }
      showRollResultToast({ result: parsed.result, actorName: actor?.name, comment: isSecret ? t('player.roll.secret', 'Скрытый бросок') : expr });
      hapticTap('light');
    } catch {
  ...
  }, [actor?.name, auth.actorId, auth.token, canRollNow, expression, isSecret, t]);

  // ── Die button handlers ──

  const dieDown = useCallback((e: React.PointerEvent<HTMLButtonElement>, die: DieType, startAngle: number) => {
    const el = e.currentTarget;
    const pid = e.pointerId;
    startPos.current = { x: e.clientX, y: e.clientY };
    longPressActive.current = false;
    pressCancel.current = false;
    pressInfo.current = { el, pid, die };
    ...
    longPressTimer.current = setTimeout(() => {
      ...
      // Init tracking from container center
      radialPrevAngle.current = null;
      radialAccum.current = 0;
      const init = getDieCount(expressionRef.current, die);
      radialBase.current = init;
      radialSnapCount.current = init;
      setRadialState({ die, count: init, startAngle });
    }, 500);
  }, []);

  const dieMove = useCallback((e: React.PointerEvent) => {
    // Cancel long-press if finger drifted too far during wait
    if (longPressTimer.current !== null) {
      ...
      if (dx * dx + dy * dy > 100) {
        ...
        longPressTimer.current = null;
        pressCancel.current = true;
      }
      return;
    }

    if (!longPressActive.current || !circleRef.current) return;

    // Track angle relative to the container center — the centre of the ring
    const rect = circleRef.current.getBoundingClientRect();
    const cx = rect.left + halfRef.current;
    const cy = rect.top + halfRef.current;
    const angle = Math.atan2(e.clientY - cy, e.clientX - cx);

    if (radialPrevAngle.current === null) {
      radialPrevAngle.current = angle;
      return;
    }
    let delta = angle - radialPrevAngle.current;
    if (delta > Math.PI) delta -= 2 * Math.PI;
    if (delta < -Math.PI) delta += 2 * Math.PI;
    radialAccum.current += delta;
    radialPrevAngle.current = angle;

    const STEP = (2 * Math.PI) / 10;
    const nc = Math.max(0, Math.min(99, radialBase.current + Math.round(radialAccum.current / STEP)));
    if (nc !== radialSnapCount.current) {
      hapticTap('light');
      radialSnapCount.current = nc;
      setRadialState(prev => prev ? { ...prev, count: nc } : null);
    }
  }, []);

  const dieUp = useCallback((die: DieType) => {
    if (longPressTimer.current !== null) {
      clearTimeout(longPressTimer.current);
      longPressTimer.current = null;
    }
    if (longPressActive.current) {
      // Confirm count into formula
      const finalCount = radialSnapCount.current;
      setExpression(prev => setDieCount(prev, die, finalCount));
      setRadialState(null);
      radialPrevAngle.current = null;
    } else if (!pressCancel.current) {
      hapticTap('light');
      setExpression(prev => addDie(prev, die));
    }
    longPressActive.current = false;
    pressCancel.current = false;
  ...
  const dieCancel = useCallback(() => {
    if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; }
    if (longPressActive.current) { setRadialState(null); radialPrevAngle.current = null; }
    longPressActive.current = false;
    pressCancel.current = false;
    pressInfo.current = null;
  }, []);
  ...
  const handleNumpadKey = useCallback((key: NumpadKey) => {
    hapticTap('light');
    if (key === '⌫') {
      numpadActiveRef.current = false;
      if (numpadTimerRef.current) { clearTimeout(numpadTimerRef.current); numpadTimerRef.current = null; }
      setExpression(prev => {
        if (!prev) return '';
    ...
    if (key === '+') {
      ...
      setExpression(prev => {
        ...
        if (!s || s.endsWith('+') || s.endsWith('-')) return prev;
        return `${s} + `;
      });
      return;
    }
    const isActive = numpadActiveRef.current;
    setExpression(prev => {
      if (isActive) return prev + key;
      const s = prev.trim();
      if (!s) return key;
      if (s.endsWith('+') || s.endsWith('-')) return prev.trimEnd() + ' ' + key;
      return `${s} + ${key}`;
    });
    numpadActiveRef.current = true;
    if (numpadTimerRef.current) clearTimeout(numpadTimerRef.current);
  ...
  // Sizing
  const half = sz / 2;
  const circleR = sz * 0.36;
  const btnSz = Math.round(Math.max(50, Math.min(70, sz * 0.19)));
  const iconSz = Math.round(btnSz * 0.52);
  const toggleSz = 44;
  const numpadPad = 8;
  const numpadTop = Math.round(sz * 0.06) + toggleSz + numpadPad;
  const isRadial = radialState !== null;

  return (
    <div className="flex flex-col">
      {/* Header */}
      <div className="px-4 pt-4 pb-2 flex items-center gap-3 shrink-0">
        <div className="w-10 h-10 rounded-xl bg-zinc-900/60 border border-zinc-800 grid place-items-center text-amber-300">
          <Dices size={20} />
        </div>
        <div className="min-w-0">
          <div className="text-sm font-semibold text-zinc-100">{t('player.dice.title', 'Набор кубов')}</div>
          <div className="text-xs text-zinc-500 truncate">{actor?.name ? `для ${actor.name}` : '—'}</div>
        </div>
        <button
          type="button"
          onClick={() => { hapticTap('light'); setIsSecret(v => !v); }}
          className={[
            'ml-auto w-10 h-10 grid place-items-center rounded-xl border transition-colors',
            isSecret
              ? 'bg-amber-500/15 border-amber-500/40 text-amber-300'
              : 'bg-zinc-900/60 border-zinc-800 text-zinc-500 hover:text-zinc-200 hover:border-zinc-700',
          ].join(' ')}
      ...
      <div className="px-4 py-2">
        <div ref={circleRef} className="relative w-full aspect-square">

          {/* ── DICE BUTTONS (hidden while radial ring is active) ── */}
          {DICE.map((die, i) => {
            const angle = (i / DICE.length) * 2 * Math.PI - Math.PI / 2;
            const x = half + circleR * Math.cos(angle) - btnSz / 2;
            const y = half + circleR * Math.sin(angle) - btnSz / 2;
            const cnt = getDieCount(expression, die);
            return (
              <button
                key={die}
                type="button"
                className={[
                  'absolute rounded-full flex flex-col items-center justify-center touch-none select-none transition-all',
                  // Hidden in numpad mode or while radial is open
                  mode === 'dice' && !isRadial ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none',
                  cnt > 0
                    ? 'bg-amber-500/20 border border-amber-500/50 text-amber-200'
                    : 'bg-zinc-900/70 border border-zinc-800 text-zinc-300',
                ].join(' ')}
                style={{ left: x, top: y, width: btnSz, height: btnSz, transition: 'opacity 0.15s' }}
                onPointerDown={(e) => dieDown(e, die, angle)}
                onPointerMove={dieMove}
                onPointerUp={() => dieUp(die)}
                onPointerCancel={dieCancel}
                onContextMenu={(e) => e.preventDefault()}
              >
                <DiceIcon die={die} size={iconSz} />
                <span className="text-[9px] font-semibold leading-none mt-0.5 opacity-70">{die}</span>
                {cnt > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-amber-500 text-black text-[9px] font-bold grid place-items-center leading-none">
                    {cnt}
                  </span>
                )}
              </button>
          ...
          {/* ── RADIAL RING (replaces die buttons) ── */}
          {isRadial && mode === 'dice' && (
            <RadialRing
              half={half}
              circleR={circleR}
              btnSz={btnSz}
              die={radialState!.die}
              count={radialState!.count}
              startAngle={radialState!.startAngle}
            />
          )}

          {/* ── NUMPAD ── */}
          <div
            className="absolute grid grid-cols-3 grid-rows-4 gap-2"
            style={{
              left: numpadPad, right: numpadPad, top: numpadTop, bottom: numpadPad,
              opacity: mode === 'numpad' ? 1 : 0,
              pointerEvents: mode === 'numpad' ? 'auto' : 'none',
          ...
          >
            {NUMPAD_KEYS.map((k) => (
              <button
                ...
                type="button"
                className={[
                  'w-full h-full rounded-2xl border font-semibold select-none active:scale-95',
                  k === '⌫'
                    ? 'bg-zinc-900/80 border-zinc-700/60 text-rose-400 text-xl'
                    : k === '+'
                      ? 'bg-zinc-900/80 border-zinc-700/60 text-emerald-400 text-2xl'
                      : 'bg-zinc-800/70 border-zinc-700/80 text-zinc-100 text-2xl',
                ].join(' ')}
                style={{ transition: 'transform 0.08s' }}
                onPointerDown={() => handleNumpadKey(k)}
              >
                {k}
              </button>
            ))}
          ...
          {/* ── CENTER TOGGLE (hidden while radial ring shows its own centre) ── */}
          {!isRadial && (
            <button
              type="button"
              onClick={() => { hapticTap('light'); setMode(m => m === 'dice' ? 'numpad' : 'dice'); }}
              className={[
                'absolute z-10 rounded-full border flex items-center justify-center transition-colors',
                mode === 'numpad'
              ...
              style={{
                width: toggleSz, height: toggleSz,
                left: half - toggleSz / 2,
                top: mode === 'dice' ? half - toggleSz / 2 : Math.round(sz * 0.06),
                transition: 'top 0.22s cubic-bezier(0.4,0,0.2,1), background-color 0.15s, border-color 0.15s',
              }}
            >
              {mode === 'dice'
                ? <span className="text-[11px] font-bold tracking-tight">123</span>
                : <Dices size={18} />
              }
            </button>
          )}
      ...
      {/* ── STICKY BOTTOM BAR ── */}
      <div className="sticky bottom-0 bg-zinc-950/95 backdrop-blur-sm border-t border-zinc-800/60 px-4 pt-3 pb-6 space-y-2.5 shrink-0">
        <div className="flex items-center justify-between gap-2">
          <div className="text-xs text-zinc-500">{t('player.dice.formula', 'Формула')}</div>
          <button
            type="button"
            onClick={() => { hapticTap('light'); setExpression(''); }}
            className="inline-flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-200 transition-colors"
          >
            <Delete size={13} /> {t('player.dice.clear', 'Очистить')}
          </button>
        </div>
        <input
          value={expression}
          ...
          inputMode="text"
          placeholder="3d6 + 1d20"
          className="w-full rounded-xl bg-zinc-900 border border-zinc-800 px-4 py-2.5 text-sm text-zinc-100 font-mono outline-none focus:border-emerald-500 transition-colors"
        />
        <button
          type="button"
          onClick={() => void roll()}
          disabled={!canRoll || statusForPending === 'pending'}
          className={[
            'w-full rounded-xl py-3.5 text-sm font-semibold transition-colors border',
            canRoll && statusForPending !== 'pending'
        ...
        >
          {statusForPending === 'pending'
            ? 'Ожидание мастера…'
            : rolling
              ? t('player.dice.rolling', 'Бросаем…')
              : t('player.dice.roll', 'Бросить')}
        </button>
        {pendingId && statusForPending === 'denied' && (
          <div className="text-xs text-rose-300/90">Мастер отклонил бросок</div>
        )}
      </div>
    </div>
  );
